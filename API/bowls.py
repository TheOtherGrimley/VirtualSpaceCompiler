...
class BowlDataset(utils.Dataset):
    def load_Bowl(self,dataset_dir,subset):
    ...
    def load_mask(self,image_id):
        ...
        #Make sure it's a Bowl we're looking at
        if image_info['source'] != "bowl":
...
def train(model):
    ...
    dataset_train.load_Bowl(args.dataset, "train")
    dataset_train.prepare()

    # Validation dataset
    dataset_val = BowlDataset()
    dataset_val.load_Bowl(args.dataset, "val")
    dataset_val.prepare()
...
if __name__ == '__main__':
    ...
    parser=argparse.ArgumentParser(
        description='Train R-CNN for bowl detection [prototype]')
    ...
    if args.command == 'train':
        config=BowlConfig() #This overrides some values from the default config.py for M-RCNN
    else:
